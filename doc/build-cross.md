Cross-compiliation of POINTS Core
===============================

POINTS Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building POINTS.
...
Note that for WSL the POINTS Core source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the POINTS Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the POINTS Core source code).
