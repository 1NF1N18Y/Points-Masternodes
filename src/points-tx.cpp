...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s points-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  points-tx [options] <hex-tx> [commands]  Update hex-encoded points transaction\n" +
              "  points-tx [options] -create [commands]   Create hex-encoded points transaction\n" +
              "\n";
